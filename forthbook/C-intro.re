 ...
 * あと 2 つの、数値として認識できない文字列は「@<strong>{ワード}(word)」として解釈される。
...
@<list>{arith}の説明に戻るが、インタプリタの挙動は以下のようになる。
 ...
 2. @<code>{+} を読み込んだインタプリタは、ワード名とその定義を紐付けて保存している「辞書(dictionary)@<fn>{dictionary}」からその定義を見つけ出し、呼び出す。@<code>{+} ワードはスタックの一番上に積まれている 2 つの数 @<code>{3}, @<code>{4} をポップし、和 @<code>{7} をプッシュする。
 3. @<code>{*} を読み込んだインタプリタは同様に定義を見つけ出して呼び出す。@<code>{*} ワードはスタックの一番上に積まれている 2 つの数 @<code>{6}, @<code>{7} をポップし、積 @<code>{42} をプッシュする。
...
扱っていく。@<code>{bye} ワードを呼び出せば Gforth を終了できる。

== スタック操作ワード

FORTH にはスタックの内容を操作するための組み込みワードが用意されている。
その 1 つが @<code>{DUP} @<fn>{dup} ワードだ。
...
@<code>{.}(ドット)ワードを呼び出すと、スタックの一番上の要素をポップして出力する。
つまり @<code>{DUP} ワードによって複製が行われスタック上にはふたつの 8 が残り、

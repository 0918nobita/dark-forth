...
ここからは、実際に定数や変数、ワードを辞書に書き込ませてみながら、
辞書の扱い方や意義について探っていくことにしよう。

=== 定数

定数は、@<b>{呼び出されるとその値をデータスタックに積むワード}として表現される。
そのようなワードを定義するには、@<hidx>{CONSTANT}@<code>{CONSTANT ( x -- )} ワードを用いる。
先に初期値をデータスタックにプッシュしておいて、直後のソースコード中に定数名を記述する。
...
変数は、@<b>{呼び出されるとデータスタックに参照先アドレスを積むワード}として表現される。
そのようなワードを定義するには、@<hidx>{VALUE}@<code>{VALUE ( x -- )} ワードを用いる。
@<code>{CONSTANT} ワードと同様に、データスタックのトップの要素を初期値、
直後のソースコード中の１単語を変数名として変数を宣言・初期化する。
「データスタックのトップの要素をアドレスとしてポップして、
書き込まれている値をプッシュする」@<hidx>{@}@<code>{@ ( a-addr -- x )} ワードを通じて、
代入されている値を取得できる。

@<list>{variable} では、@<code>{270} を初期値として変数 @<code>{bar} を宣言・初期化している。
代入には @<hidx>{!}@<code>{!( x a-addr -- )} ワードを用いる。
...
変数の宣言だけをして、初期化を後回しにするには @<hidx>{VARIABLE}@<code>{VARIABLE} ワードを用いる。
...
実用上は、未使用のメモリ空間の先頭から、指定した容量を確保する @<hidx>{ALLOT}@<code>{ALLOT ( n -- )} ワードと併用される。
慣用的には、@<list>{create-allot} のように @<code>{CREATE} ワードの呼び出しと @<code>{ALLOT} ワードの呼び出しを連続させることが多い。
参照先アドレスを得るためのワードの名前と、実際に確保されているメモリ領域のサイズが見てすぐに把握できるからだ。

//list[create-allot][CREATE, ALLOT ワードを用いて 1 セル変数を生成する][forth]{
...
//emlist[実行結果]{
6FFFF87DDD8: F4 01 00 00  00 00 00 00 -
500
//}

空きデータ域の先頭アドレスを取得するだけであれば @<hidx>{HERE}@<code>{HERE ( -- addr )} ワードで呼び出せば可能であるから、
...
先述した @<code>{!} ワードを用いる方法と、新たに登場する @<code>{,} (カンマ) ワードを用いる方法がある。
...
配列は「要素の値が連続して書き込まれたメモリ領域の先頭アドレス」として表現される。
...
@<hidx>{' (ティック)}@<code>{'} (ティック) ワードを用いて既定義ワードの XT を取得できる。
XT をもとに実際に呼び出しを発生させるには @<hidx>{EXECUTE}@<code>{EXECUTE ( i * x xt -- j * x )} ワードを用いる。
...
実際に匿名ワードを生成するには @<hidx>{:NONAME}@<code>{:NONAME} ワードを用いる。@<code>{:NONAME} の後ろに、@<code>{:} と
...
//list[defer-error][定義内容の紐付いていないワードを呼び出す実験][forth]{
DEFER foo
foo
//}
...
それを可能にする 1 つの方法として、ここでは @<hidx>{DOES>}@<code>{DOES>} ワードを紹介する。

@<code>{DOES>} ワードは、辞書に追加された最後の項目の実行時意味論を上書きする。
...
@<code>{foo} ワードの実行時意味論は、 @<code>{DOES>} ～ @<code>{;} の前後で
...
「参照先の値を取得する」という実行時意味論で上書きするために @<code>{DOES>} と @<code>{;} の間で
...
もう１つ、@<code>{DOES>} ワードの利用例を紹介する。
@<list>{counter} では、呼び出すたびにカウントアップされた値を返すワードを定義している。

//list[counter][毎回カウントアップされた値を返すワード]{
CREATE counter -1 ,
DOES> DUP DUP @ 1 + SWAP ! @ ;
counter .
counter .
...
@<code>{DOES>} ワードの追加する実行時意味論である「参照先アドレスの取得」を利用して、
呼び出すたびに参照先の値を更新することも可能である。
そのため参照先のメモリ領域は、そのワードにとっての環境 (各局所変数とその値の対応) として利用できる。
@<code>{counter} ワードは、いわばクロージャ (関数閉包) だったというわけだ。

@<code>{VARIABLE}, @<code>{VALUE} ワードによって定義されたワードでも、
@<code>{DOES>} ワードによる実行時意味論の上書きが可能である。

//list[value-does][VALUE によって作られる変数に対しての DOES>][forth]{
57 VALUE prime-num
DOES> @ 3 / ;
prime-num .
//}
